...
const getPageIDFromBlockChildren = async (
    blockId: string,
    pages: Set<string>,
    route: string[]
): Promise<{ pageId: string; route: string[] }[]> => {
    ...
    if (!!pageName && typeof pageName === "string")
        route.push(pageName.toLowerCase());
    ...
    const nestedPages = await Promise.all(
        Array.from(childPagesSet).map(async (blockId) => {
            return await getPageIDFromBlockChildren(
                blockId as string,
                pages,
                Array.from(route)
            );
...
export const getSiteMap = async (blockId: string) => {
    ...
    const pageIds = await getPageIDFromBlockChildren(
        blockId,
        new Set([blockId]),
        []
    );

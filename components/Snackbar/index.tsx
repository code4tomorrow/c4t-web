...
const Snack : React.FC<{ children: ReactNode, i: number, id: string }> = ({ children, i, id }) => {
    ...
    useEffect(() => {
        ...
        const tl = gsap.timeline({
            ...
            setTimeout(() => isUnmounting.current = true, RESET_DELAY_MS);
                ...
                setSnackbar((snacks) => snacks.filter((snack) => snack.key !== id));
        });
    ...
    return (
        <div ref={containerRef} className="bg-dark-grey-primary opacity-0 left-1/2 -translate-x-1/2 p-3 my-1 absolute rounded-full border

...
const FullJob: React.FC<IFullJobProps> = ({ preview, internship = false }) => {
    const { data: queriedJob, error } = useSWR<IJob>(
        getAPIJobByID(preview.sys?.id),
        {
            fetcher,
        }
    );
        ...
        >
            ...
            >
                ...
                <a
                    ...
                    target={"_blank"}
                    href={
                        internship ? INTERNSHIP_SIGNUP_LINK : MEMBER_SIGNUP_LINK
                    }
                    ...
                    {!isLoadingDebounced ? (
                        ...
                        >
                            {job.responsibilities?.json ? (
                                <Document
                                    document={job.responsibilities.json}
                                />
                            ) : (
                                <p className="text-medium-grey">N/A</p>
                            )}
                                ...
                                <div className="[&>*]:mr-2 text-sm font-semibold">
                                    {job.realLifeJobs?.map((realLifeJob, i) => (
                                        <span
                                            key={i}
                                            style={{
                                                background: `linear-gradient(to right, ${factionColor}, #7892EE)`,
                                            }}
                                            className={clsx(
